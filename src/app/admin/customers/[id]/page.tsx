...
import { ArrowRight, Mail, Phone, MapPin, ShoppingCart, DollarSign, Clock3, Building2, MoreHorizontal } from "lucide-react";
            ...
            <div className=" grid gap-8 xl:grid-cols-[1.4fr_420px] ">
                ...
                <div className="space-y-8">
                    ...
                    <div className=" rounded-[34px] border border-slate-200 bg-white p-8 ">
                        ...
                        <textarea rows={7} placeholder="یادداشت تیم فروش..." className=" mt-8 w-full resize-none rounded-2xl border border-slate-200 bg-slate-50 p-5 text-sm outline-none " />
                        <button className=" mt-6 rounded-2xl bg-blue-600 px-6 py-4 text-sm font-medium text-white ">ذخیره یادداشت</button>
                    </div>
                </div>
                {/* RIGHT */}
...
function TH({ children }: { children: ReactNode }) {
    return (
        <th className=" px-8 py-5 text-sm font-bold text-slate-700 ">{children}</th>
    );
}
    ...
    return (
        <tr className=" border-t border-slate-100 ">
            ...
            <td className="px-8 py-6">
                <div className=" inline-flex rounded-full bg-emerald-50 px-4 py-2 text-xs font-medium text-emerald-700 ">{status}</div>
            </td>
        </tr>
    );
}
...
function AccountRow({
    ...
    return (
        ...
        <div className=" flex items-center justify-between rounded-2xl bg-slate-50 p-5 ">
            ...
            <p className="font-bold text-slate-900">{value}
            </p>
        </div>
    );
}