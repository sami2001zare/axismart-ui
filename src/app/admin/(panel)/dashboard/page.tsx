                ...
                <div className="relative z-10 flex flex-col gap-8 xl:flex-row xl:items-center xl:justify-between">
                    <div>
                        ...
                        <p className="mt-6 max-w-2xl text-sm leading-8 text-blue-100">
                            نمای کلی فروشگاه، فروش، سفارشات، موجودی و عملکرد تجاری.
                        </p>
                    </div>

                    <div className="grid gap-4 sm:grid-cols-2">
                        <HeroStat label="Revenue" value="۱۲۵M" />

                        <HeroStat label="Orders" value="842" />

                        <HeroStat label="Customers" value="4,921" />

                        <HeroStat label="Products" value="248" />
                    </div>
                </div>
            </div>
                ...
                <div className="space-y-8">
                    ...
                    <div className="rounded-[34px] border border-slate-200 bg-white p-8">
                        <div className="flex items-center justify-between">
                            <div>
                                <h3 className="text-xl font-bold text-slate-900">عملکرد فروش</h3>

                                <p className="mt-2 text-sm text-slate-500">۶ ماه اخیر</p>
                            </div>
                        ...
                        <div className="mt-12 flex h-80 items-end gap-5">
                            {[55, 82, 63, 95, 72, 88].map((h) => (
                                <div key={h} className="flex flex-1 flex-col items-center gap-4">
                                    ...
                                    <p className="text-xs font-medium text-slate-500">ماه</p>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="overflow-hidden rounded-[34px] border border-slate-200 bg-white">
                        <div className="border-b border-slate-100 px-8 py-6">
                            <h3 className="text-lg font-bold text-slate-900">آخرین سفارشات</h3>
                        </div>
                ...
                <div className="space-y-8">
                    ...
                    <div className="rounded-[34px] border border-slate-200 bg-white p-7">
                        ...
                        <div className="mt-7 space-y-4">
                            <ActionBtn>محصول جدید</ActionBtn>

                            <ActionBtn>سفارشات</ActionBtn>

                            <ActionBtn>مدیریت مشتریان</ActionBtn>
                        </div>
                    </div>
                    ...
                    <div className="rounded-[34px] border border-slate-200 bg-white p-7">
                        <h3 className="font-bold text-slate-900">هشدارها</h3>

                        <div className="mt-8 space-y-5">
                            <AlertItem title="۷ محصول کم‌موجودی" color="amber" />

                            <AlertItem title="۳ سفارش در انتظار تایید" color="blue" />

                            <AlertItem title="۱ پرداخت ناموفق" color="red" />
                        </div>
                    </div>
                    ...
                    <div className="rounded-[34px] border border-slate-200 bg-white p-7">
                        <h3 className="font-bold text-slate-900">فعالیت اخیر</h3>

                        <div className="mt-8 space-y-6">
                            <Activity icon={<Clock3 size={16} />} title="محصول جدید اضافه شد" />
                            ...
                            <Activity icon={<Package size={16} />} title="موجودی بروزرسانی شد" />
                        </div>
                    </div>
                </div>
            </div>
...
function OrderRow({
    ...
    return (
        <tr className="border-t border-slate-100">
            ...
            <td className="px-8 py-6">{customer}</td>
...
function ActionBtn({ children }: { children: ReactNode }) {
    return (
        <button className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4 text-right text-sm font-medium transition hover:bg-blue-600 hover:text-white">
            {children}
        </button>
    );
}

function AlertItem({ title, color }: { title: string; color: string }) {
    const colors: {
        amber: string;
        blue: string;
        red: string;
...
function Activity({ icon, title }: { icon: ReactNode; title: string }) {
    return (
