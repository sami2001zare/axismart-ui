    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
                <div>
                    <h1 className="text-4xl font-black text-slate-900">اعلان‌ها</h1>
                ...
                <div className="flex flex-wrap gap-4">
                    <button className="inline-flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-5 py-4 text-sm font-medium">
                        <CheckCheck size={16} />
                        خواندن همه
                    </button>

                    <button className="inline-flex items-center gap-3 rounded-2xl border border-red-200 bg-red-50 px-5 py-4 text-sm font-medium text-red-600">
                        <Trash2 size={16} />
                        پاکسازی
                    </button>
                </div>
            ...
            <div className="rounded-[34px] border border-slate-200 bg-white p-6">
                <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
                    ...
                    <div className="flex flex-wrap gap-4">
                        <Action icon={<Filter size={15} />}>فیلتر</Action>

                        <Action icon={<Settings2 size={15} />}>تنظیمات</Action>
                    </div>
                </div>
            </div>
...
function NotificationCard({
    ...
    return (
        <div
            className={`rounded-[34px] border bg-white p-7 transition ${unread ? 'border-blue-300' : 'border-slate-200'}`}
        >
                ...
                <div className="flex items-start gap-5">
                    ...
                    <div>
                        <div className="flex flex-wrap items-center gap-4">
                            ...
                            {unread && (
                                <div className="rounded-full bg-blue-100 px-4 py-2 text-[11px] font-bold text-blue-700">
                                    جدید
                                </div>
                            )}
                        </div>
                        <p className="mt-4 text-sm leading-8 text-slate-500">{desc}</p>
...
function Action({ children, icon }: { children: ReactNode; icon: ReactNode }) {
    return (
